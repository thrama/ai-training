...
# Specifically get prediction for [74, 5, 10, 2, 100]
test_case = np.array([74, 5, 10, 2, 100])
h1_in = np.dot(test_case, w0) + b0
h1_out = hidden_activation(h1_in)
h2_in = np.dot(h1_out, w1) + b1
h2_out = hidden_activation(h2_in)
out_in = np.dot(h2_out, w2) + b2
final_prediction = output_activation(out_in)
print(f"\nPrediction for [74, 5, 10, 2, 100]: ${final_prediction[0]:.2f}")